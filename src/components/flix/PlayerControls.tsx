  ...
  return (
    <div className="pointer-events-none absolute inset-0 flex flex-col justify-between bg-gradient-to-b from-black/70 via-transparent to-black/85">
      <div className="pointer-events-auto flex items-center gap-4 p-4 md:p-8">
        <button type="button" onClick={onClose} aria-label="Fermer le lecteur" className="-ml-1.5 rounded-full p-1.5 text-white transition-colors hover:bg-white/10">
      ...
      <div className="pointer-events-auto space-y-2 p-4 md:px-8 md:pb-6">
        ...
        <div className="flex flex-wrap items-center gap-4">
          <button type="button" onClick={onTogglePlay} aria-label={playing ? "Mettre en pause" : "Lecture"} className="rounded-full p-1.5 text-white transition-colors hover:bg-white/10">
            {playing ? <Pause className="size-7 fill-white" /> : <Play className="size-7 fill-white" />}
          </button>
          <button type="button" onClick={() => onSeekRelative(-10)} aria-label="Reculer de 10 secondes" className="rounded-full p-1.5 text-white transition-colors hover:bg-white/10">
            <RotateCcw className="size-6" />
          </button>
          <button type="button" onClick={() => onSeekRelative(10)} aria-label="Avancer de 10 secondes" className="rounded-full p-1.5 text-white transition-colors hover:bg-white/10">
            <RotateCw className="size-6" />
          </button>
          <div className="group flex items-center gap-2">
            <button type="button" onClick={onToggleMute} aria-label={muted ? "Réactiver le son" : "Couper le son"} className="rounded-full p-1.5 text-white transition-colors hover:bg-white/10">
            ...
            <input
              ...
              aria-label="Volume"
              className="w-16 accent-accent md:w-24"
            />
          </div>
          ...
          <div className="ml-auto flex items-center gap-4">
            {trackMenuAvailable && (
              <button type="button" onClick={onToggleTrackMenu} aria-label="Pistes audio et sous-titres" className="rounded-full p-1.5 text-white transition-colors hover:bg-white/10">
                <Captions className="size-6" />
              </button>
            )}
            <button type="button" onClick={onToggleFullscreen} aria-label={fullscreen ? "Quitter le plein écran" : "Plein écran"} className="rounded-full p-1.5 text-white transition-colors hover:bg-white/10">
